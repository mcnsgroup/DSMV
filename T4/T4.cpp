...
/**< @brief Duration of the pulse to indicate a successful (or unsuccessful) processing of the serial buffer. */
int pulseTime = 0;
...
double T4getTime() {
	...
	timer += counter - timerRefreshed;	// Update global timer
	timerRefreshed = counter;			// Timer has been updated with current GPT1 value
	return (double) (timer) / T4ccRate;	// return time in seconds
}
...
void T4checkSerialBuffer() {
	...
	delay(pulseTime);			// Delay to make the pulse on the pin (success or fail) visible
	T4dw(FailPin, LOW);			// Set pin to indicate that processing a command failed to LOW
